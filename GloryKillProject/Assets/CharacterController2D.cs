...
public class CharacterController2D : MonoBehaviour
{
	...
	private bool m_OnWall;				//Whether or not the player is on a wall.
	...
	private void FixedUpdate()
	{
		...
		// The player is grounded if a circlecast to the groundcheck position hits anything designated as ground
		// This can be done using layers instead but Sample Assets will not overwrite your project settings.
		Collider2D[] collidersW = Physics2D.OverlapCircleAll(m_WallCheck.position, k_GroundedRadius, m_WhatIsWall);
		...
		if (jump)
		{
			...
			else
			{
				if (m_OnWall)
				{
					...
					m_Grounded = false;
					m_Rigidbody2D.AddForce(new Vector2(-m_WallJumpForceX * Mathf.Cos(transform.rotation.eulerAngles.y * Mathf.Deg2Rad), m_WallJumpForceY));
